...
class ClusterWithGlusterfs:
    ...
    ERROR_FAIL_TO_CREATE_SEED_INSTANCE = 6
    ERROR_FAIL_TO_CREATE_SERVICE = 7
    ERROR_FAIL_TO_CREATE_JOINING_INSTANCE = 8
    ERROR_FAIL_TO_GET_OWNING_REPLICATION_CONTROLLER_LIST = 9
    ERROR_FAIL_TO_DELETE_SERVICE = 10
    ERROR_FAIL_TO_DELETE_REPLICATION_CONTROLLER = 11
    ERROR_FAIL_TO_DELETE_POD = 12
...
class ElasticSearch(ClusterWithGlusterfs):
    def __init__(self):
        ...
        if self.action == "create":
            ...
            self.joining_instance_up_keyword_list = []
...
if elasticSearch.action == "create":
    status_code = elasticSearch.create_cluster()
    if status_code != ClusterWithGlusterfs.SUCEESS:
        elasticSearch.clean_cluster()
        sys.exit(status_code)
elif elasticSearch.action == "resize":
    status_code = elasticSearch.resize_cluster()
    sys.exit(status_code)
elif elasticSearch.action == "delete":
    status_code = elasticSearch.clean_cluster()
    sys.exit(status_code)
else:
