...
class Cassandra(ClusterWithGlusterfs):
    def __init__(self):
        ...
        if self.action == "create":
            ...
            self.volume_to_mount = "cassandra-volume"
            self.seed_instance_up_keyword_list = ["Listening for thrift clients"]
            self.joining_instance_up_keyword_list = ["Handshaking version with", "is now part of the cluster", "Listening for thrift clients"]
            self.joining_node_failure_keyword_list = ["Exception encountered during startup", "Announcing shutdown"]
        elif self.action == "resize":
            # Set environment
            for i in xrange(0, len(self.replication_controller_dictionary["spec"]["template"]["spec"]["containers"])):
                for j in xrange(0, len(self.replication_controller_dictionary["spec"]["template"]["spec"]["containers"][i]["env"])):
