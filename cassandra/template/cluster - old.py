...
class Cassandra(Cluster):
    def __init__(self):
        ...
        self.seed_node_up_keyword_list = ["Listening for thrift clients"]
        self.all_node_up_keyword_list = ["Handshaking version with", "is now part of the cluster", "Listening for thrift clients"]
        self.all_node_failure_keyword_list = ["Exception encountered during startup", "Announcing shutdown"]


cassandra = Cassandra()
if cassandra.action == "create":
    if not cassandra.create_cluster():
        cassandra.clean_cluster()
        sys.exit(-1)
elif cassandra.action == "clean":
    cassandra.clean_cluster()
elif cassandra.action == "resize":
    cassandra.resize_cluster()
else:
